...
genres.each do |genre|
	...
	albids.each do |albid|
		songs = DB.execute("SELECT songtitle, artist, lyrics_ml, lyrics_w FROM master WHERE album_id = ? AND ((lyrics_w NOTNULL AND lyrics_w != '') OR (lyrics_ml NOTNULL AND lyrics_ml != ''))", albid['album_id'])
		puts songs.count 
		if songs.count >= 1
			then
